...
func main() {
	...
	fmt.Println("Hello")
	// Indicamos que vamos a agregar 1 Gorutine al WaitGroup para que espere su ejecucion
	//antes de que la gurutine base (main) muera, y así le de tiempo a la siguiente gorutine de ejecutarse
	wg.Add(1)
	// la palabra reservada go ejecutará la funcion de forma concurrente
	go say("world", &wg)

	// Funcion del WaitGroup que sirve para decirle al gorutine principal (main) que espere hasta que todas
	// las gorutine del WaitGroup finalicen, es decir, hasta que se ejecute 'defer wg.Done()' en cada una de las goroutines
	wg.Wait()
